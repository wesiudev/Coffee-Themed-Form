...
function App() {
  const [{ userName, isSugar, isMilk }, setCredentials] =
    useState<CredsState>({
      userName: "",
      isSugar: false,
      isMilk: false,
    ...
    <div className="wrapper">
      <div className="wrapper__content">
        ...
        <form onSubmit={submitForm} action="">
          ...
          {isSubmitted ? (
          <div className={isSubmitted ? "wrapper__content__receipt receipt-animate" : ""}>
            <div>
            <h3>Receipt</h3>
            <h5>{date}</h5>
            ...
            <br />
            <code>Item: {coffeeId}.{coffeeName}</code>
            </div>
        ...
        <div className="wrapper__content__buttons">
          ...
          {!isSubmitted ? <button
            onClick={currentStep === 3 ? handleSubmit : () => handleStepByButton("increment")}
            style={setButtonStyles()}
            className="submitBtn"
          >
            {currentStep === 3 ? "Submit" : "Next"}
          </button> : <div style={{textAlign: 'center', color: 'rgb(236, 236, 236)', marginTop:'35px'}}>Have a nice day<br/>Zapraszamy ponownie{" :)"}</div>}
        </div>
